...
impl<B: BlockT + 'static, H: ExHashT> NetworkBackend<B, H> for Litep2pNetworkBackend {
		...
		Self: Sized,
	{
		...
		let mut config_builder = Self::configure_transport(&params.network_config);
		...
		config_builder = config_builder
			.with_known_addresses(known_addresses.into_iter())
		...
		let listen_addresses = litep2p.listen_addresses().cloned().collect();
